<script lang="ts">
	...
	let quoteAuthorTitle = $state('');

	const htmlValue = $derived(`
  <section class="not-prose full-width isolate overflow-hidden bg-gray-100 px-6 lg:px-8 mt-14">
	<div class="relative mx-auto max-w-2xl py-10 sm:py-16 lg:max-w-5xl">
		<div
		...
		<figure class="grid grid-cols-1 items-center gap-x-6 gap-y-8 lg:gap-x-10">
			<div class="relative col-span-2 lg:col-start-1 lg:row-start-2">
				...
				<div class="text-xl font-semibold leading-8 text-gray-900 sm:text-2xl sm:leading-9">
					<div>
						${quoteText}
					</div>
				</div>
